...
class myloginview(LoginView):
    ...
    def dispatch(self, request, *args, **kwargs):
        request.session['type'] = self.template_name
...
def home_view(request):
    ...
    return render(request,'blood/index.html', context= None)
...
def afterlogin_view(request, **kwargs):
    if request.session.get('type') is not None:
        type = request.session.get('type').split('/')[0]

        if is_donor(request.user) and type[0] == 'd':
            return redirect('donor/donor-dashboard')

        elif is_patient(request.user) and type[0] == 'p':
        ...
        return render(request,'blood/index.html',context={'error' : 'User is not ' + type.capitalize() })
    print('####################')
    print(request.user.is_authenticated)
